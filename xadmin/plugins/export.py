...
class PrintMenuPlugin(BaseAdminPlugin):
    ...
    def block_top_toolbar(self, context, nodes):
        if self.list_Print:
            # context.update({
            #     'show_export_all': self.admin_view.paginator.count > self.admin_view.list_per_page and not ALL_VAR in self.admin_view.request.GET,
            #     'form_params': self.admin_view.get_form_params({'_do_': 'print'}, ('export_type',)),
            #     'export_types': [{'type': et, 'name': self.export_names[et]} for et in self.list_Print],
            # })
            contextdict = get_context_dict(context)
            print('get_context_dict(context)',contextdict['verbose_name'])
            relist = []
            for i in contextdict['results']:
                ii = i['object']
                # print('i',i.object)
                # for o in i.cells:
                #     print('o',str(o))
                hasflag = 0
                for j in relist:
                    if ii.prov.fullname() == j['name']:
                        j['num'] = j['num'] + 1
                        hasflag = 1
                        break
                if hasflag == 0:
                    pnum = {'name': ii.prov.fullname(), 'num': 1}
                    relist.append(pnum)
            print('relist', relist)
            context.update({
                'relist': relist,
            })
            nodes.append(loader.render_to_string('xadmin/blocks/model_list.top_toolbar.print.html',
                                                 context=get_context_dict(context)))
...
class PrintPlugin(BaseAdminPlugin):
...
site.register_plugin(PrintMenuPlugin, ListAdminView)
